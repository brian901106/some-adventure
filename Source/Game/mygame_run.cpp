...
void CGameStateRun::OnMove()							// 移動遊戲元素
{
	//挑戰3
	if (phase == 3) {
		if (character.Top() + character.Height() >= chest_and_key.Top() && character.Left() + character.Width() >= chest_and_key.Left()) {
	...
	//挑戰5
	if (phase == 5) {
		if (character.Top() + character.Height() >= door[0].Top() && character.Left() + character.Width() >= door[0].Left()) {
			door[0].SelectShowBitmap(1);
		}
		if (character.Top() + character.Height() >= door[1].Top() && character.Left() + character.Width() >= door[1].Left()) {
			door[1].SelectShowBitmap(1);
		}
		if (character.Top() + character.Height() >= door[2].Top() && character.Left() + character.Width() >= door[2].Left()) {
	...
	//挑戰6
	if (phase == 6) {
		if (ball.GetSelectShowBitmap() == 0 && ball.IsAnimation() == false) {
...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt , UINT nFlags )
{
	...
	if (nChar == VK_RIGHT) {
		x = character.Left();
		y = character.Top();
		character.SetTopLeft(x + speed, y);
	}
	if (nChar == VK_LEFT) {
		x = character.Left();
		y = character.Top();
	...
	if (nChar == VK_UP) {
		x = character.Left();
		y = character.Top();
		character.SetTopLeft(x, y - speed);
	}
	if (nChar == VK_DOWN) {
		x = character.Left();
		y = character.Top();
